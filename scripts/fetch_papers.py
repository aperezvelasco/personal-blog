...
def fetch_arxiv_papers(query: str, max_results: int = 15) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for entry in root.findall('atom:entry', ns):
            ...
            paper_id: str = paper_id_elem.text.strip() if paper_id_elem is not None else ""
            
            # Extract title and clean whitespace
            title_elem: Optional[ET.Element] = entry.find('atom:title', ns)
            title: str = " ".join(title_elem.text.split()) if title_elem is not None else "No Title"
            
            # Extract summary (abstract)
            summary_elem: Optional[ET.Element] = entry.find('atom:summary', ns)
            summary: str = " ".join(summary_elem.text.split()) if summary_elem is not None else ""
            
            # Extract published date
            published_elem: Optional[ET.Element] = entry.find('atom:published', ns)
            published: str = published_elem.text.strip() if published_elem is not None else ""
...
def categorize_paper(title: str, summary: str) -> str:
    ...
    scores: Dict[str, int] = {
        "Weather Forecasting": text.count("weather forecast") + text.count("weather prediction") + text.count("forecasting") * 0.5,
        "Subseasonal to Seasonal Forecasting": text.count("subseasonal") + text.count("seasonal forecasting") + text.count("s2s"),
        "Climate Emulation": text.count("emulator") + text.count("emulation") + text.count("climate model emulator"),
        "Data Assimilation": text.count("data assimilation") + text.count("state estimation"),
        "Downscaling": text.count("downscaling") + text.count("super-resolution") + text.count("spatial resolution")
    }
    
    # Pick the category with the highest non-zero score, default to "Weather Forecasting"
...
def is_relevant_geoscience(title: str, summary: str) -> bool:
    ...
    # Blacklist keywords
    blacklist: List[str] = [
        "space weather", "solar flare", "radiation belt", "geomagnetic", "magnetosphere",
        "retinopathy", "medical", "interatomic", "molecule", "molecular", "materials science",
        "crystallography", "chemistry", "quantum chemistry", "astronomy", "cosmology",
        "galaxy", "stellar", "brain", "clinical", "biomedical", "polymer", "catalysis",
        "protein", "drug", "nanoparticle", "ionosphere", "coronal"
    ]
    ...
    # Whitelist keywords
    whitelist: List[str] = [
        "weather", "climate", "climatology", "meteorology", "meteorological", 
        "precipitation", "rainfall", "downscaling", "temperature", "forecast", 
        "projection", "reanalysis", "wind", "monsoon", "el nino", "la nina", 
        "sea surface temperature", "emulation", "emulator", "data assimilation", 
        "nowcasting", "atmosphere", "atmospheric", "oceanic", "hydrology", "hydrological"
    ]
...
def get_weekly_papers() -> List[Dict[str, Any]]:
    ...
    q_weather: str = 'abs:"weather forecasting" AND (abs:"deep learning" OR abs:"machine learning" OR abs:"neural network")'
    # 2. S2S forecasting query
    q_s2s: str = '(abs:"subseasonal" OR abs:"seasonal forecast") AND (abs:"deep learning" OR abs:"machine learning" OR abs:"neural network")'
    # 3. Climate emulation query
    q_emulation: str = '(abs:"climate emulation" OR abs:"climate emulator" OR abs:"model emulator") AND (abs:"deep learning" OR abs:"machine learning" OR abs:"neural network")'
    # 4. Data Assimilation query
    q_assimilation: str = 'abs:"data assimilation" AND (abs:"deep learning" OR abs:"machine learning" OR abs:"neural network")'
    # 5. Downscaling query
    q_downscaling: str = 'abs:"downscaling" AND (abs:"deep learning" OR abs:"machine learning" OR abs:"neural network")'
    ...
    for default_category, query in queries.items():
        ...
        for paper in fetched:
            ...
            if cat == "Weather Forecasting" and not ("weather" in (paper["title"] + " " + paper["summary"]).lower()):
